...
# as the name suggests, it does the whole processing of the image, with the window with sliders for better tunning
...
def apply_processing(image, method, blur_ksize, thresh_val, block_size, C, morph_ksize):
    ...
    if method == 0:
        ...
        binary = cv2.adaptiveThreshold(blurred, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, block_size, C)
...
# I was having trouble with the separation of objects. A lot of screws were being counted as one.
# so I created a function to apply watershed segmentation to separate the objects better.

# as you can see, it is 'disabled' if the min_distance_val is less than 1.

def watershed_apply(binary_image, min_distance_val):
...
def main():
    ...
    # this part is responsible for creating the csv file, with all data gathered and classified by the user.
    ...
    with open(OUTPUT_CSV_FILE, mode='w', newline='') as csv_file:
        ...
        for image_name in file_names:
            ...
            print(f"\n--- AJUSTANDO IMAGEM: {image_name} ---") #better recognition
            ...
            while True:
                ...
                labels = watershed_apply(binary_img, watershed_dist)
